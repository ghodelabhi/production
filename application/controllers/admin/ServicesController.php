...
class ServicesController extends CI_Controller {
	...
	public function index(){
		...
		$data['title'] = "Calibration Certificate";
		$data['content'] = 'admin/cc';
		$data['getCategory'] = $this->ServicesModel->getCC(); //retrive all category
	...
	public function addEditCC(){
		if (isset($this->session->userdata['logged_in'])){
			$category_id = $this->uri->segment(4);// get id form url
			if($category_id==''){
				$data['title'] = "Add Calibration Certificate";
				
			}else{
				$data['title'] = "Edit Calibration Certificate";
			}
			$data['content'] = 'admin/addeditcc';
